...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("pixcoin"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("pixcoin://"))
    {
        uri.replace(0, 11, "pixcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Pixcoin.lnk";
}
...
{
    return GetAutostartDir() / "pixcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Pixcoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  pixcoin-qt [" + tr("command-line options") + "]                     " + "\n";
